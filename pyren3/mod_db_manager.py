...
def get_file_from_clip(filename):
    ...
    if mod_globals.os == "android" or mod_globals.clip_arc != "":
        mode = "r"
    ...
    if mod_globals.clip_arc == "":
        ...
        if mode == "rb":
            ...
            return open(
                os.path.join(mod_globals.cliproot, filename), mode, encoding=encoding
            )
    else:
...
def extract_from_clip_to_cache(filename):
    if mod_globals.clip_arc == "":
        ...
        target = open(
            os.path.join(mod_globals.cache_dir, os.path.basename(filename)), "wb"
        )
        with source, target:
