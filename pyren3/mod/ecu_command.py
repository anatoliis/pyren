...
def runCommand(command, ecu, elm, param="", cmdt="HEX"):
        ...
        for dr in command.datarefs:
            ...
            if dr.type == "State":
                datastr, help, csvd = get_state(
                    ecu.States[dr.name], ecu.Mnemonics, ecu.Services, ecu.elm, ecu.calc
                )
            if dr.type == "Parameter":
                datastr, help, csvd = get_parameter(
                    ecu.Parameters[dr.name],
                    ecu.Mnemonics,
                    ecu.Services,
                    ecu.elm,
                    ecu.calc,
            ...
            if dr.type == "Identification":
                datastr, help = get_identification(
                    ecu.Identifications[dr.name],
                    ecu.Mnemonics,
                    ecu.Services,
                    ecu.elm,
                    ecu.calc,
    ...
    for si in command.serviceID:
        ...
        if len(service.params) == 1 and chosenParameter == "":
            ...
            if (
                cmdt == "HEX"
                and all(c in string.hexdigits for c in ch)
                and len(ch) % 2 == 0
            ):
            ...
            if cmdt == "DEC" and all(c in string.digits for c in ch):
                ...
                if parsize > 0 and len(chosenParameter) < parsize * 2:
                    chosenParameter = (
                        "0" * (parsize * 2 - len(chosenParameter)) + chosenParameter
                    )
            if cmdt == "ASCII":
...
def executeCommand(command, ecu, elm, path):
    ...
    if (isParam - isInputList) != 0:
        ...
        cmdt = input("Press ENTER to exit or type [HEX, DEC, ASCII, VIN]: ")
        cmdt = cmdt.upper()
    ...
    ch = input("Are you ready to execute the command? <yes/no>:")
    if ch.lower() != "yes":
        return
    ...
    if (
        len(dataid)
        and dataid in list(ecu.DataIds.keys())
        and len(mnemo)
        and mnemo in list(ecu.Mnemonics.keys())
        and len(chosenParameter) > 0
    ):
    ...
    for si in command.serviceID:
        ...
        if len(service.params) == 1 and chosenParameter == "":
            ...
            print(
                "Parametr type:",
                service.params[0]["type"],
                " size:",
                service.params[0]["size"],
            )
            while True:
                ch = input(cmdt + ":")
                ch = ch.strip().upper()
