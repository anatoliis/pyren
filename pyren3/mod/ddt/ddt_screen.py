...
class ButtonConfirmationDialog(tkinter.simpledialog.Dialog):
    def __init__(self, parent, text):
        ...
        txt = tk.Text(
            self.top, relief=tk.GROOVE, borderwidth=1, wrap="none"
        )  # , width=190, heigth=180
...
class FindDialog(tkinter.simpledialog.Dialog):
    def __init__(self, parent, ecu):
        ...
        self.pattern = ""
        self.findEnt = tk.Entry(
            topFrame, relief=tk.GROOVE, borderwidth=1, textvariable=self.pattern
        )
...
class DDTScreen(tk.Frame):
    ...
    def __init__(self, ddtFileName, xdoc, decu, top=False):
        ...
        self.expertmode.set(config.EXPERT_MODE)
    ...
    def updateScreenValues(self, req, rsp):
        ...
        for request in request_list:
            if any(
                key in list(request.ReceivedDI.keys())
                for key in list(self.dValue.keys())
            ) or any(
                key in list(request.ReceivedDI.keys())
                for key in list(self.iValue.keys())
            ):
                r = request
            ...
            if d in list(self.iValue.keys()) and self.iValueNeedUpdate[d]:
                if self.prefer_ECU.get():
                    if (
                        ...
                        or self.decu.datas[d].BytesASCII
    ...
    def updateScreen(self):
        ...
        if self.decu.rotaryCommandsQueue.empty():
            ...
            for r in list(self.dReq.keys()):
                ...
                if (req[:2] not in AllowedList) and not config.EXPERT_MODE:
                    tmstr = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    self.addToLog(
    ...
    def startScreen(self):
        ...
        for r in self.sReq_lst:
            ...
            # not expert mode protection
            if (req[:2] not in ["10"] + AllowedList) and not config.EXPERT_MODE:
            ...
            for c in slist:
                ...
                if len(list(r.ReceivedDI.keys())):
                    ...
                    for d in list(r.ReceivedDI.keys()):
                        val = self.decu.getValue(
                            d, False, r, rsp
                        )  # allow execute commands with manuelSend
                        if d in list(self.dValue.keys()):
                            if ":" in val:
                                self.dValue[d].set(val.split(":")[1])
                            else:
    ...
    def resizeEvent(self, event):
        ...
        if self.currentscreen is not None:
            self.jdsu = self.root.after(
                500, lambda: self.loadScreen(self.currentscreen)
            )
    ...
    def askDumpName(self):
        ...
        for root, dirs, files in os.walk("./dumps"):
            for f in files:
                if self.decu.ecufname.split("/")[-1][:-4] in f:
                    try:
                        uda = f.split("_")[0]
                        fda = datetime.datetime.fromtimestamp(int(uda)).strftime(
                            "%Y/%m/%d %H:%M:%S"
                        )
                        flist.append(fda + "\t#\t" + f)
                    except:
                        tkinter.messagebox.showinfo(
                            "Wrong dump file",
                            "Wrong file name " + f + " in ./dumps folder",
                        )
                        return ""
        # if len (flist) == 0:
        ...
        try:
            ...
            tkinter.messagebox.showinfo(
                "Wrong dump file", "No appropriate dump file in ./dumps folder"
            )
            return ""
        # check dump file
    ...
    def dumpRollBack(self):
        ...
        savedMode = config.DEMO
        config.DEMO = True
        saveDumpName = config.DUMP_NAME
        self.decu.loadDump(fname)

        (conf_1, cv_1) = self.decu.makeConf(indump=True)

        config.DUMP_NAME = saveDumpName
        config.DEMO = savedMode
        if config.DEMO:
            self.decu.loadDump(config.DUMP_NAME)
        else:
            self.decu.clearELMcache()
    ...
    def showDiff(self):
        ...
        if config.DEMO:
            xText += "> " + self.dumpName2str(config.DUMP_NAME) + "\n\n"
        else:
        ...
        for i in aK:
            ...
            if (
                (i not in list(cv_1.keys()))
                or (i not in list(cv_2.keys()))
                or cv_1[i] != cv_2[i]
            ):
                flag = False
                xText += "-" * 30 + "\n%s\n" % (i)
    ...
    def changeMode(self):
        config.EXPERT_MODE = self.expertmode.get()
        if config.EXPERT_MODE:
            ...
            self.modeLabel.configure(
                text="ReadOnly Mode", background="#d9d9d9", foreground="#000000"
            )

    def changeInpPref(self):
        if self.prefer_ECU.get():
            self.inpPrefLabel.configure(
                text="Inputs derived from ECU",
                background="#d9d9d9",
                foreground="#000000",
            )
        else:
            self.inpPrefLabel.configure(
                text="Inputs derived from XML",
                background="#d9d9d9",
                foreground="#000000",
            )
            # request to update dInputs
    ...
    def exit(self):
        ...
        try:
            ...
            pass
        try:
            if self.prefer_ECU is not None:
                del self.prefer_ECU
        except:
            pass
        try:
            ...
            pass
        try:
            if self.ScreensMenu is not None:
                del self.ScreensMenu
        except:
            pass
        try:
            if self.dDisplay is not None:
                del self.dDisplay
        except:
            pass
        try:
            ...
            pass
        try:
            if self.iValue is not None:
                del self.iValue
        except:
            pass
        try:
            if self.iValueNeedUpdate is not None:
                del self.iValueNeedUpdate
        except:
            pass
        try:
            ...
            pass
        try:
            if self.sReq_dl is not None:
                del self.sReq_dl
        except:
            pass
        try:
            if self.sReq_lst is not None:
                del self.sReq_lst
        except:
            pass
        try:
            if self.dBtnSend is not None:
                del self.dBtnSend
        except:
            pass
        try:
            if self.dFrames is not None:
                del self.dFrames
        except:
            pass
        try:
    ...
    def readDTC(self):
        tkinter.messagebox.showinfo(
            "Read DTC", "Under construction. Use pyren screens."
        )

    def clearDTC(self):
        tkinter.messagebox.showinfo(
            "Clear DTC", "Under construction. Use pyren screens."
        )

    def rightButtonClicked(self, event, tag=""):
        id = self.ddt.find_closest(event.x, event.y)[0]

        if id not in list(self.tObj.keys()) and tag == "":
            return
        ...
        r = ""
        if closest in list(self.decu.req4data.keys()) and self.decu.req4data[
            closest
    ...
    def initUI(self):
        ...
        self.toolsmenu.add_command(
            label="Make torque PIDs", command=self.torqpids, accelerator="Ctrl+t"
        )
        self.toolsmenu.add_command(
            label="Find", command=self.find, accelerator="Ctrl+f"
        )
        self.toolsmenu.add_command(
            label="Clear logs", command=self.clearLogs, accelerator="Ctrl+k"
        )
        self.toolsmenu.add_command(
            label="Save logs to file", command=self.saveLogs, accelerator="Ctrl+s"
        )
        ...
        self.settingsmenu.add_checkbutton(
            ...
            onvalue=True,
            offvalue=False,
            variable=self.translated,
            command=self.repaint,
        )
        self.settingsmenu.add_separator()
        self.settingsmenu.add_checkbutton(
            label="Prefer Inputs from ECU",
            onvalue=True,
            offvalue=False,
            variable=self.prefer_ECU,
            command=self.changeInpPref,
        )
        self.settingsmenu.add_separator()
        self.settingsmenu.add_checkbutton(
            ...
            for cat in categs:
                ...
                if len(screens):
                    for scr in screens:
                        ...
                        catmenu.add_command(
                            label=scrname,
                            command=lambda item=scrname: self.OnScreenChange(item),
                        )
        self.menubar.add_cascade(label="Screens", menu=self.ecumenu)
        self.root.config(menu=self.menubar)

        self.lrpw = tk.PanedWindow(
            self.top_f,
        ...
        self.startStopButton = tkinter.ttk.Button(
            btnFrame, text="Stop", command=self.startStop
        )
        ...
        # commands tree
        self.tree.insert(
            "", "end", "ddt_all_commands", text="ddt_all_commands", open=False
        )
        ...
        self.vbar = tk.Scrollbar(
            self.rig_f, orient=tk.VERTICAL, command=self.ddtcnv.yview
        )
        ...
        self.ddtfrm_id = self.ddtcnv.create_window(
            (0, 0), window=self.ddtfrm, anchor=tk.NW
        )
    ...
    def loadScreen(self, scr):
        ...
        self.sReq_lst = (
            []
        )  # list of requests sent thru starting the screen (order is important)
        ...
        if _minInputHeight == 0:
            _minInputHeight = 10
        if _minButtonHeight == 0:
            _minButtonHeight = 20
        ...
        self.ddt.config(
            width=max_x // scx, height=max_y // scy, bg=self.ddtColor(bg_color)
        )
        self.ddtcnv.xview_moveto(0)
        self.ddtcnv.yview_moveto(0)
            ...
            for lab in sorted(slab, key=lambda k: k["sq"], reverse=True):
                ...
                if "::pic:" in xText:
                    self.ddt.create_rectangle(
                        ...
                        xrLeft + xrWidth,
                        xrTop + xrHeight,
                        fill=self.ddtColor(65535),
                        outline=self.ddtColor(0),
                    )
                ...
                if "::pic:" not in xText:
                    ...
                    if gifname:
                        ...
                        self.images[-1] = self.images[-1].subsample(
                            x1 * 3 // xrWidth, y1 * 3 // xrHeight
                        )
                        idg = self.ddt.create_image(
                            xrLeft, xrTop, image=self.images[-1], anchor="nw"
                        )
        ...
        if len(dispalys):
            for dispaly in dispalys:
                ...
                obj.bind(
                    os_event,
                    lambda event, tag=xText: self.rightButtonClicked(event, tag),
                )
            ...
            for input in inputs:
                ...
                if xText not in list(self.iValue.keys()):
                    ...
                    if xText in list(self.decu.req4data.keys()) and self.decu.req4data[
                        ...
                        self.dReq[self.decu.req4data[xText]] = self.decu.requests[
                            self.decu.req4data[xText]
                        ].ManuelSend
                    self.iValue[xText] = tk.StringVar()
                    self.iValueNeedUpdate[xText] = True

                if xText in list(self.decu.datas.keys()) and len(
                    list(self.decu.datas[xText].List.keys())
                ):
                    optionList = []
                    if xText not in list(self.iValue.keys()):
                        self.iValue[xText] = tk.StringVar()
                    ...
                    for i in list(self.decu.datas[xText].List.keys()):
                        optionList.append(
                            hex(int(i)).replace("0x", "").upper()
                            + ":"
                            + self.decu.datas[xText].List[i]
                        )
                    ...
                    obj = tk.Entry(
                        ...
                        borderwidth=1,
                        font=lFont,
                        textvariable=self.iValue[xText],
                    )
            ...
            for button in buttons:
                ...
                if "::btn:" not in xText:
                    obj = tk.Button(
                        ...
                        font=lFont,
                        relief=tk.GROOVE,
                        command=lambda key=str(slist), btn=xText: self.buttonPressed(
                            btn, key
                        ),
                    )
                else:
                    ...
                    if gifname:
                        ...
                        self.images[-1] = self.images[-1].zoom(3, 3)
                        self.images[-1] = self.images[-1].subsample(
                            x1 * 3 // xrWidth + 1, y1 * 3 // xrHeight + 1
                        )
                        obj = tk.Button(
                            frame,
                            image=self.images[-1],
                            font=lFont,
                            relief=tk.GROOVE,
                            command=lambda key=str(
                                slist
                            ), btn=xText: self.buttonPressed(btn, key),
                        )
    ...
    def loadSyntheticScreen(self, rq):
        ...
        self.ddt.create_rectangle(
            max_x * 0.01, 10, max_x * 0.98, 60, fill="yellow", outline="gold"
        )
        self.ddt.create_text(
            max_x // 2, 50, text=xText, width=max_x, anchor="s", font=lFont
        )

        xfSize = str(int(float(10) * self.scf))
        lFont = tkinter.font.Font(family="Arial", size=xfSize)
        pn = 0
        for xText, zzz in sorted(
            list(self.decu.requests[rq].ReceivedDI.items()),
            key=lambda item: item[1].FirstByte,
        ):
            ...
            frame = tk.Frame(
                self.ddt, width=max_x // 2, height=35, relief=tk.GROOVE, borderwidth=0
            )
            ...
            obj.bind(
                os_event, lambda event, tag=xText: self.rightButtonClicked(event, tag)
            )
