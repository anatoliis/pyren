...
class DataItem:
    ...
    def __str__(self):
        out = "Endian = %-5s FirstByte = %2d BitOffset = %2d  Ref = %1d Name = %s" % (
            self.Endian,
            self.FirstByte,
            self.BitOffset,
            self.Ref,
...
class DecuRequest:
  ...
  ReplyBytes            = %s
    ...
    def __init__(self, rq, defaultEndian):
        ...
        if DossierMaintenabilite:
            self.DossierMaintenabilite = True

        self.ManuelSend = False
        ManuelSend = rq.findall("ns0:ManuelSend", ns)
        if ManuelSend:
            self.ManuelSend = True
        ...
        if len(Sent):
            ...
            if len(SentBytes):
                self.SentBytes = SentBytes[0].text  # [0].text
        ...
        if len(Received):
            ...
            if len(ShiftBytesCount):
                self.ShiftBytesCount = int(ShiftBytesCount[0].text)  # [0].text)

            self.ReplyBytes = ""
            ReplyBytes = Received[0].findall("ns0:ReplyBytes", ns)
            if len(ReplyBytes):
                self.ReplyBytes = ReplyBytes[0].text  # [0].text
...
class DecuRequests:
    def __init__(self, requiest_list, xdoc):
        ...
        tmpdoc = xdoc.findall(
            "ns0:Target/ns0:Requests", ns
        )  # getElementsByTagName("Requests")
        defaultEndian = ""
