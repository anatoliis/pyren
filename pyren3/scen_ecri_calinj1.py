...
import mod_globals
from mod_utils import ASCIITOHEX, clearScreen, pyren_encode
...
def run(elm, ecu, command, data):
    ...
    if isHEX == 1 and not (
        all(c in prmCHAR for c in chk.upper()) and (len(chk) == nbCC * 4)
    ):
        print("*" * 80)
        ch = input("Hexdata check failed. Press ENTER to exit")
        return
    elif isHEX == 0 and not (
        all(c in prmCHAR for c in chk.upper()) and (len(chk) == nbCC * 4)
    ):
        print("*" * 80)
    ...
    if isHEX == 1:
        ...
        print("*" * 80)
        ch = input(
            "!!!!!!!!There is a bug somwhere in the scenario, operation aborted!!!!!!!!!"
        )
        return
