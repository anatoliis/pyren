...
class ScanEcus:
    ...
    def __init__(self, elm_ref):
        ...
        for file in file_list:
            ...
            if vh.hasAttribute("defaultText"):
                ...
                self.vhcls.append(
                    [vehiclename, file, vehTypeCode, vehTCOM, vehindexTopo]
                )
    ...
    def scanAllEcus(self):
        ...
        if os.path.isfile(SEFname) and not mod_globals.opt_scan:
            ...
            # debug
            # check vehTypeCode
            if len(self.detectedEcus) > 0 and "vehTypeCode" not in list(
                self.detectedEcus[0].keys()
            ):
                ...
                self.detectedEcus = sorted(
                    self.detectedEcus, key=lambda k: int(k["idf"])
                )
            ...
            if len(self.detectedEcus) > 0 and "idTx" not in list(
                self.detectedEcus[0].keys()
            ):
                # renew savedEcus
                self.allecus = {}
        ...
        for ecu, row in sorted(
            ...
            if (
                self.allecus[ecu]["pin"] == "can"
                and self.allecus[ecu]["pin1"] == canH
                and self.allecus[ecu]["pin2"] == canL
            ):
                i = i + 1
                print(
                    "\r"
        ...
        if not mod_globals.opt_can2:
            ...
            for ecu, row in sorted(
                ...
                if (
                    self.allecus[ecu]["pin"] == "iso"
                    and self.allecus[ecu]["pin1"] == "7"
                    and self.allecus[ecu]["pin2"] == "15"
                ):
                ...
                if name in list(self.allecus.keys()) or all:
                    ...
                    if EcuData.getElementsByTagName("OptimizerId").item(0).firstChild:
                        self.allecus[name]["OptimizerId"] = (
                            EcuData.getElementsByTagName("OptimizerId")
                            .item(0)
                            .firstChild.nodeValue.strip()
                        )
                    else:
                    ...
                    if ecui:
                        ...
                        fastinit = ""
                        fastinit_tag = ecui.item(0).getElementsByTagName(
                            "FastInitAddress"
                        )
                        if fastinit_tag:
                        ...
                        slowinit = ""
                        slowinit_tag = ecui.item(0).getElementsByTagName(
                            "SlowInitAddress"
                        )
                        if slowinit_tag:
                        ...
                        replaytorequestdelay_tag = ecui.item(0).getElementsByTagName(
                            "ReplyToRequestDelay"
                        )
                        if replaytorequestdelay_tag:
                            replaytorequestdelay = replaytorequestdelay_tag.item(
                                0
                            ).getAttribute("value")
                        ...
                        programmable = ""
                        programmable_tag = ecui.item(0).getElementsByTagName(
                            "Programmable"
                        )
                        if programmable_tag:
                        ...
                        if frms:
                            ...
                            if KeepAlive:
                                self.allecus[name]["keepAliveReq"] = KeepAlive.item(
                                    0
                                ).getAttribute("request")
                                self.allecus[name]["KeepAlivePeriod"] = KeepAlive.item(
                                    0
                                ).getAttribute("period")
                            else:
...
def readECUIds(elm):
    ...
    if len(IdRsp) > 59:
        ...
        try:
            ...
            if (
                len(IdRsp_F18A) > 8
                and "NR" not in IdRsp_F18A
                and "BUS" not in IdRsp_F18A
            ):
            ...
            if (
                len(IdRsp_F194) > 8
                and "NR" not in IdRsp_F194
                and "BUS" not in IdRsp_F194
            ):
            ...
            if (
                len(IdRsp_F195) > 8
                and "NR" not in IdRsp_F195
                and "BUS" not in IdRsp_F195
            ):
                                ...
                                if bus[:1] != "7":
                                    if faddr in addreses[bus].keys():
                                        faddr = (
                                            faddr
                                            + "#"
                                            + addreses[bus][faddr]["idTx"]
                                            + "#"
                                            + addreses[bus][faddr]["idRx"]
                                        )
                                    else:
                                        faddr = faddr + "##"
