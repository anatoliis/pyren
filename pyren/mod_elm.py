    ...
    def __init__(self, portName, speed, portTimeout):
        ...
        if re.match(
            ...
            if len(self.droid.bluetoothActiveConnections().result) != 0:
                self.btcid = list(
                    self.droid.bluetoothActiveConnections().result.keys()
                )[0]
    ...
    def __init__(self, portName, speed, log, startSession="10C0"):
        ...
        if elm_rsp and "?" not in elm_rsp:
            ...
            try:
                ...
                version_number = int(
                    "".join([re.sub("\D", "", version) for version in firmware_version])
                )
    ...
    def send_can_cfc(self, command):
        ...
        while Fc < Fn:
            ...
            while cf > 0:
                ...
                if (tc - tb) * 1000.0 < ST:
                    target_time = time.clock() + (ST / 1000.0 - (tc - tb))
    ...
    def send_raw(self, command):
        ...
        while True:
            ...
            if command in self.buff:
                ...
                tmstr = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                self.lf.write(
                    "<[" + tmstr + "]" + self.buff + "(shifted)" + command + "\n"
                )
                self.lf.flush()
    ...
    def checkModulePerformaceLevel(self, dataids):
        ...
        if self.performanceModeLevel == 3 and mod_globals.opt_obdlink:
            for level in reversed(
                list(range(4, 100))
            ):  # (number_of_dids_to_get_multiframe_request, max_performance_level_to_check - 1)
                is_level_accepted = self.check_performance_level(level, dataids)
                if is_level_accepted:
                    return
    ...
    def check_performance_level(self, level, dataids):
        if len(dataids) >= level:
            ...
            while did_number < len(dataids) and len(param_to_send) / 4 < level:
                # get another did
                did = list(dataids.keys())[did_number]
                did_number += 1
